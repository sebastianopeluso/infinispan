...
public abstract class AbstractGMUTest extends MultipleCacheManagersTest {
   ...
   protected final void assertNoTransactions() {
   ...
   protected final TransactionTable transactionTable(int cacheIndex) {
      return advancedCache(cacheIndex).getComponentRegistry().getComponent(TransactionTable.class);
   }

   protected final void assertEventuallyEquals(final int cacheIndex, final Object key, final Object value) {
      eventually(new Condition() {
         @Override
         public boolean isSatisfied() throws Exception {
