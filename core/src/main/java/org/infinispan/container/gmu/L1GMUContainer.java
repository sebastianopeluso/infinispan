...
public class L1GMUContainer {
   ...
   private boolean isValid(L1Entry entry, EntryVersion txVersion, Address owner) {
      long entryVersionValue = toGMUEntryVersion(entry.getReadVersion()).getVersionValue(owner);
      long txEntryVersionValue = toGMUEntryVersion(txVersion).getVersionValue(owner);

      return entryVersionValue == GMUEntryVersion.NON_EXISTING || txEntryVersionValue == GMUEntryVersion.NON_EXISTING ||
