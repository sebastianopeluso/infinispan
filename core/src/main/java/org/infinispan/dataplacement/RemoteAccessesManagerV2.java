 ...
 * @since 5.2
 */
public class RemoteAccessesManagerV2 {
   private static final Log log = LogFactory.getLog(RemoteAccessesManagerV2.class);
   ...
   public RemoteAccessesManagerV2(DistributionManager distributionManager, DataContainer dataContainer) {
   ...
   private void calculateAccessesIfNeeded(){
      ...
      Map<Object, Long> tempAccesses = analyticsBean.getTopKFrom(StreamLibContainer.Stat.REMOTE_GET);

      log.info("Size of Remote Get is: " + tempAccesses.size());

      // Only send statistics if there are enough objects
      if (tempAccesses.size() >= analyticsBean.getCapacity() * 0.8) {
         remoteAccessPerAddress.putAll(sortObjectsByPrimaryOwner(tempAccesses));
      }

      tempAccesses = analyticsBean.getTopKFrom(StreamLibContainer.Stat.LOCAL_GET);
    ...
    */
   private ConsistentHash getConsistentHashingFunction() {
