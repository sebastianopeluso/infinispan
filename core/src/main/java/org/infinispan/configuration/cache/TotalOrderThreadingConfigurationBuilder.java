...
public class TotalOrderThreadingConfigurationBuilder extends AbstractTransportConfigurationChildBuilder<TotalOrderThreadingConfiguration> {
   ...
   void validate() {
      ...
      boolean distribution = clustering().cacheMode().isDistributed();
      if (distribution && onePhaseCommit) {
         throw new ConfigurationException("One Phase Commit not allowed with DISTRIBUTION");
      }

      if (distribution && locking().isolationLevel == IsolationLevel.REPEATABLE_READ && locking().writeSkewCheck) {
         throw new ConfigurationException("Distribution in TOM-based scheme doesn't allow Repetable Read with " +
               "Write skew");
      }
